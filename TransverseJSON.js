...
// prints top level headings
    for (var c in obj) {
        if (obj[c] == "[object Object]" || Array.isArray(obj[c])) {
            ...
            document.getElementById("displayJson").innerHTML += "<div id="+ c +"><h2 class='h2'>" + c + "</h2></div>" + loopMore(JSON.stringify(obj[c])) + "<br>";
        } else {
            // show the value of the JSON object
            document.getElementById("displayJson").innerHTML += "<div id="+ c +"><h2 class='h2'>" + c + "</h2></div>" + "<p>" + obj[c] + "</p>";
        }
    }
    ...
    function loopMore(objText) {
        ...
        for (var c in json) {
            if (json[c] == "[object Object]") { // transverse further through the JSON object array
                if (isNaN(c)) {
                    str += "<br><h3 class='h3'>" + c + "</h3> " + loopMore(JSON.stringify(json[c])) + " ";
                }else {// is the start of an array of [objects Objects]
                    for(var x in json[c]){
                       str += "<br><h3 class='h3'>" + x + "</h3> "; // show the property name
                ...
                if (isNaN(c)) {
                    ...
                    str += "<br><h3 class='h3'>" + c + "</h3><br> " + "<p>" + json[c] + "</p>";
                } else {
